...
*♻️Barang                       |💲Harga beli*\n` +
'```' + `
${rpg.emoticon('bgl')}Blue Gem Lock          :  ${Bbgl}`.trim() +
'```'
+ '\n\n*♻️Barang                       | 💲Harga Jual*\n' +
'```' + `
${rpg.emoticon('bgl')}Blue Gem Lock          :  ${Sbgl}
`.trim() + '```'
    try {
            ...
            case 'buy':
                switch (_type) {
                    case 'limit':
                         ...
                         break
                    case 'bgl':
                            if (global.db.data.users[m.sender].gems >= bgl * count) {
                                global.db.data.users[m.sender].gems -= bgl * count
                                global.db.data.users[m.sender].bgl += count * 1
                                conn.sendButton(m.chat, `Succes membeli ${count} ${rpg.emoticon('bgl')}BGL dengan harga ${potion * count} ${rpg.emoticon('money')}money\n\nGunakan ${rpg.emoticon('bgl')}BGL dengan ketik: *${usedPrefix}use potion <jumlah>*`, wm, `Gunakan ${rpg.emoticon('potion')}Potion`, usedPrefix + 'heal', m)
                            } else conn.send2Button(m.chat, `Gems anda tidak cukup untuk membeli ${count} ${rpg.emoticon('bgl')}Potion dengan harga ${Bbgl * count} ${rpg.emoticon('bgl')}bgl`, wm, 'inventory', usedPrefix + 'inv', 'claim', usedPrefix + 'claim', m)
                        break
                    case 'potion':
                ...
                switch (_type) {
                    ...
                    case 'bgl':
                        if (global.db.data.users[m.sender].bgl >= count * 1) {
                            ...
                            global.db.data.users[m.sender].bgl -= count * 1
                            conn.reply(m.chat, `Succes menjual ${count} Kayu dengan harga ${Sbgl * count} Gems`.trim(), m)
            ...
            switch (type) {
                ...
                case 'bgl':
                        if (global.db.data.users[m.sender].gems >= Bbgl * count) {
                            ...
                            conn.sendButton(m.chat, `Succes membeli ${count} ${rpg.emoticon('bgl')}BGL dengan harga ${Bbgl * count} ${rpg.emoticon('bgl')}BGL`, wm, 'inventory', usedPrefix + 'inv', m)
                        } else conn.send2Button(m.chat, `Gems anda tidak cukup untuk membeli ${count} ${rpg.emoticon('bgl')}BGL dengan harga ${Bbgl * count} ${rpg.emoticon('bgl')}BGL`, wm, 'inventory', usedPrefix + 'inv', 'claim', usedPrefix + 'claim', m)
                    break
                case 'kayu':
                        if (global.db.data.users[m.sender].money >= Bkayu * count) {
                            global.db.data.users[m.sender].kayu += count * 1
...
handler.help = ['shop <jual|beli> <args>']
handler.tags = ['gt']
    
handler.command = /^(shopgt)$/i
